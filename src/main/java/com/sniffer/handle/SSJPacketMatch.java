...
public class SSJPacketMatch {
	 ...
	 */
	public static SSJPacketMatch getInstance() {
		if (mpm == null) {
			mpm = new SSJPacketMatch();
		}
		return mpm;
	}
	...
	public void handlePacket(PcapPacket packet) {
		SSJPacketMatch.totalOfIp += packet.getTotalSize() / (1024.0 * 1024.0);// 单位：MB
	...
	private void handleIp4(PcapPacket packet) {
		...
		if (SSJPacketMatch.int2Ip(ip4.destinationToInt()).equals("255.255.255.255")) {
			// System.out.println("收到了一个广播包");
			SSJUI.lItems.add(numberOfPacket, "广播数据包");
	...
	private void handleTcp(PcapPacket packet) {
		...
		SSJUI.lItems.add(numberOfPacket, "TCP Packet");
	...
	private void handleUdp(PcapPacket packet) {
		...
		SSJUI.lItems.add(numberOfPacket, "UDP Packet");
	...
	private void handleArp(PcapPacket packet) {
		...
		SSJUI.lItems.add(numberOfPacket, "ARP Packet");
	...
	private void handleIcmp(PcapPacket packet) {
		...
		SSJUI.lItems.add(numberOfPacket, "ICMP Packet");
